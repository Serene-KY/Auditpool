...
export async function registerRisksRoutes(app: FastifyInstance) {
  app.get('/risks', { preHandler: [app.tenantMiddleware] }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const { rows } = await request.db.query(
        `SELECT id, tenant_id, audit_scope_id, name, description, created_at, created_by, updated_at, updated_by, version
         FROM risks WHERE tenant_id = $1 AND (is_deleted = false OR is_deleted IS NULL)
  ...
  app.post('/risks', { preHandler: [app.tenantMiddleware] }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const { rows } = await request.db.query(
        `INSERT INTO risks (tenant_id, audit_scope_id, name, description)
         VALUES ($1, $2, $3, $4)
         RETURNING id, tenant_id, audit_scope_id, name, description, created_at, created_by, updated_at, updated_by, version`,
