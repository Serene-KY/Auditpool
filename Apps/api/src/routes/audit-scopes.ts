...
export async function registerAuditScopesRoutes(app: FastifyInstance) {
  app.get(
    '/audit-scopes',
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        return reply.status(500).send({ error: 'Failed to fetch audit scopes' });
      }
    }
  );
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (pgErr.code === '23503') {
          return reply.status(400).send({ error: 'Framework not found' });
        }
        return reply.status(500).send({ error: 'Failed to create audit scope' });
      }
    }
  );
}
